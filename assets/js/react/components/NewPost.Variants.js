...
export const Variants = ({ show, brandId }) => {
  ...
  useEffect(() => {
    getChannels(brandId).then((channels) => {
  ...
  const isVariantActive = (network) => {
    if (!variantExistsForNetwork(network)) return false
  ...
  return (
    <div style={{ display: show ? 'block' : 'none' }} className="py-4">
      <p className="text-center mb-5">
        ...
        {!dateTime || !title ? null : (
          <span className="small d-block text-muted">
            ...
            {dateFormat(dateTime)} at {timeFormat(dateTime)}
          </span>
        )}
      </p>
      <Row>
            ...
            : channels.map((e, i) => {
                ...
                return (
                  <Button
                    ...
                    active={isVariantActive(e.network)}
                    onClick={() => toggleVariant(e)}
                  >
      ...
      {(!variantArray || variantArray.filter((n) => n.active).length < 1) &&
      !noChannels ? (
        <p className="lead font-weight-bold text-center mt-4">
              ...
              <Col sm={3}>
                <Nav variant="pills" className="flex-column">
                  {variantArray.map((variant, i) => {
                    ...
                    return (
                      <Nav.Item key={i}>
                        <Nav.Link eventKey={i} className="text-truncate">
                          {sentenceCase(variant.network)}
                        </Nav.Link>
                      </Nav.Item>
                    )
                  })}
    ...
    <div>
      {!showHeading ? null : (
        <Row>
          <Col>
            <h4 className="mb-3">
              {socialIcons[slug]} {sentenceCase(variant.network)}{' '}
              {!variant.username ? null : (
                <small className="text-muted">{variant.username}</small>
              )}
            </h4>
          </Col>
        </Row>
        ...
        <Col sm={5}>
          <MediaSelecter
            ...
            onSelect={(selectedImage) => {
              ...
              if (selectedImage) {
                updateVariant('post_variant_media', [
