  ...
  return (
    <div className={`calendar__day ${todayClass} ${mutedClass}`} key={date}>
      ...
      {past && !muted ? (
        ...
        <Droppable droppableId={`${!muted ? '' : '_'}${date}`}>
          {(provided, snapshot) => (
            <div
              ref={provided.innerRef}
              style={{
...
const CalendarDays = ({ year, month }) => {
  ...
  let days = Array.from(Array(daysInMonth).keys()).map((e, i) => ({
    date: i,
    muted: false,
  }))
  ...
  days = !posts.length
    ...
    : days.map((x) => {
        let todaysPosts =
          posts.filter((y) => {
            return (
              new Date(Date.parse(y.scheduled_for)).getDate() - 1 === x.date
            )
          }) || null
        x.posts = todaysPosts
        return x
  ...
  const previousMonthDays = startingDay
    ? new Array(startingDay)
        .fill()
        .map((item, index) => ({
          date: lastMonthEndingDate - index,
          muted: true,
        }))
        .reverse()
    : []
  const nextMonthDays = Array.from(Array(6 - endingDay).keys()).map((e, i) => ({
    date: i,
    muted: true,
  }))
    ...
    <>
      {!activePost ? null : (
        <EditPostModal
          post={activePost}
          show={activePost}
          handleShow={() => setActivePost(false)}
          key={key}
        />
      )}
      <DragDropContext
        onDragEnd={(result) => {
          ...
          let newDate = moment
            .utc(updatedPost.scheduled_for)
            .set('date', destination.droppableId)
            .format()
          ...
          {days.map((e, i) => {
            return (
              <CalendarDay
                ...
                posts={e.posts}
                muted={e.muted}
                past={moment(`${e.date + 1} 00:00:00`, 'DD hh:mm:ss').isBefore(
                  moment('00:00:00', 'hh:mm:ss'),
                )}
                today={
                  currentDay === new Date(year, month, e.date + 1).getTime()
                }
                date={e.date !== null ? e.date + 1 : ''}
...
const ScheduleCalendarMain = ({ brandId }) => {
  ...
  return (
    <section className="bdo-scheduleCalendar">
      <header className="d-flex align-items-center justify-content-between">
        ...
        <div>
          <Button variant="link" onClick={prevMonth}>
            <ChevronLeft />
          </Button>
          <Button variant="link" onClick={nextMonth}>
            <ChevronRight />
          </Button>
        </div>
      </header>
...
const ScheduleCalendar = ({ brandId }) => {
  let initialState = { ...initialAppState, activeBrand: { id: brandId } }
  return (
    <EIStateProvider initialState={initialState} reducer={reducer}>
      <ScheduleCalendarMain brandId={brandId} />
    </EIStateProvider>
  )
}
