...
const NewPostModal = ({ show, handleShow }) => {
  ...
  const handleSchedule = () => {
    ...
    schedulePost(activeBrand.id, body)
      .then((newPost) => {
        ...
        const bodies = [
          'Your post is scheduled. Way to go!',
          'Post scheduled. You deserve an award.',
          'That post is ready to go!',
          'Done and done. Post scheduled.',
        ]
        dispatch({
  ...
  return (
    <Modal size="lg" centered={true} show={show} onHide={handleShow}>
      ...
      <Modal.Body>
        <NewPostWizard activePage={activePage}>{pages}</NewPostWizard>
      </Modal.Body>
      <Modal.Footer className="justify-content-between">
        ...
        {activePage + 1 >= pages.length ? (
          ...
          <Button variant="primary" onClick={nextPage} disabled={!title}>
            Next ›
          </Button>
        )}
  ...
  const reducer = (state, action) => {
    switch (action.type) {
      ...
      case 'toggleVariantActive': {
        ...
        newVariants[action.network]['active'] = !state.variants[action.network][
          'active'
        ]
      ...
      case 'createVariant': {
        ...
        if (action.mediaId) {
          newVariants[action.network]['post_variant_media'] = [
