...
const MediaNavbar = ({
  ...
  const onDesignPublish = (exportUrl, designId) => {
    ...
    axios
      ...
      .then((res) => {
        createMedia(activeBrand.id, designId, res.data).then((data) => {
          dispatch({
            type: 'setAssets',
            assets: [data, ...assets],
          })
  ...
  return (
    <Navbar bg="light" {...classes()}>
      ...
      <div>
        <DropdownButton alignRight title="Add Media Items" variant="primary">
          <Dropdown.Item onClick={() => handleUploadModalShow()}>
            Upload Image/Video
          </Dropdown.Item>
          <Dropdown.Divider />
          <CanvaButton
...
const MediaDetailModal = ({
  ...
  const fileSize =
    media.bytes < 1024000
      ? `${Number(media.bytes / 1024).toFixed(2)} KB`
      : `${Number(media.bytes / 1024 / 1024).toFixed(2)} MB`
          ...
          <Col>
            <Form>
              <Form.Group controlId="editMediaTitle" as={Row}>
                <Form.Label column sm={3} {...classes('label')}>
                  Title
                </Form.Label>
                <Col>
                  <Form.Control
              ...
              <Form.Group
                controlId="editMediaUploadDate"
                as={Row}
                className="mb-0"
              >
                <Form.Label column sm={3} {...classes('label', 'small')}>
                  Uploaded
                </Form.Label>
                <Col sm="9">
                  <Form.Control
                    plaintext
                    readOnly
                    defaultValue={dateFormat(media.inserted_at)}
                    className="form-control-sm"
                  />
                </Col>
              </Form.Group>
              <Form.Group
                controlId="editMediaDimensions"
                as={Row}
                className="mb-0"
              >
                <Form.Label column sm={3} {...classes('label', 'small')}>
                ...
                <Col sm="9">
                  <Form.Control
                    plaintext
                    readOnly
                    defaultValue={`${media.width}px × ${media.height}px`}
                    className="form-control-sm"
                  />
                </Col>
              </Form.Group>
              <Form.Group
                controlId="editMediaFileSize"
                as={Row}
                className="mb-0"
              >
                <Form.Label column sm={3} {...classes('label', 'small')}>
    ...
    <>
      {showUploadModal ? (
        <MediaUploadModal
          ...
          handleShow={handleUploadModalShow}
      ...
      <section>
        ...
        <MediaDetailModal
          media={activeMedia}
          show={showEditModal}
          handleShow={handleEditModalShow}
          handleSave={(data) => handleMediaSave(data)}
          handleDelete={() => handleMediaDelete(activeMedia.id)}
        />
        <Row>
          {!filteredAssets ? (
            'Loading...'
          ) : !filteredAssets.length ? (
            <div className="p-5 mx-auto w-50 text-center h3">
              Looks like you don't have any media yet. Add some to get started!
            </div>
          ) : (
...
const Media = ({ brandId }) => {
  ...
  let initialState = { ...initialAppState, activeBrand: activeBrand }
