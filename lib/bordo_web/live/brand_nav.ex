defmodule BordoWeb.BrandNav do
  ...
  def render(assigns) do
    ~L"""
    <div class="bg-blue-900 text-purple-50 w-18 p-3 flex flex-col h-full">
      <div class="flex-1 ">
        <%= for brand <- @brands do %>
      ...
      <div class="pin-b" x-data="{ open: false }"  phx-update="ignore">
        <div class="relative z-10">
          <img @click="open = !open" @click.away="open = false" class="h-12 rounded-full" src="<%= BordoWeb.Admin.UserView.avatar(@current_user) %>" alt="" />
  ...
  defp avatar_class(active) do
    if active do
      ...
      "bg-white h-12 hover:no-underline w-12 flex items-center justify-center text-gray-500 text-2xl font-semibold rounded-lg mb-1 overflow-hidden opacity-50"
    end
  end
end
